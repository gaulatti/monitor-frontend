...
function PostCard({ post, expandedPost, setExpandedPost }: PostCardProps) {
  ...
  return (
    <div className='entry' data-relevance={post.relevance} data-accent='blue'>
      ...
      {/* Media Carousel */}
      {imageMedia.length > 0 && <MediaCarousel media={imageMedia} className='post-media-carousel' />}
...
function EventCard({ event, expandedEvent, setExpandedEvent }: EventCardProps) {
  ...
  const avgRelevance = event.posts.length > 0 ? Math.round((event.posts.reduce((sum, post) => sum + post.relevance, 0) / event.posts.length) * 10) / 10 : 0;
  ...
  return (
    <div className='entry' data-relevance={semaphoreValue} data-accent='purple'>
      <div className='entry-header'>
        ...
        <span className={`category-badge event-status status-${event.status}`}>{event.status.toUpperCase()}</span>
      </div>
      ...
      <div className='entry-meta'>
        <div className='tags'>
          <span className='tag event-count'>{event.posts_count} POSTS</span>
          <span className='tag event-relevance'>AVG {avgRelevance}</span>
          <span className='tag event-id'>#{event.id}</span>
        </div>
        <div className='entry-actions'>
...
export function Events() {
  ...
  const filteredEvents = filter
    ? events.filter((event) => event.title.toLowerCase().includes(filter.toLowerCase()) || event.summary.toLowerCase().includes(filter.toLowerCase()))
    : events;
  ...
  if (error) {
    return (
      // <DashboardLayout>
        <div className='dashboard'>
          <div style={{ color: '#ef4444', padding: '24px', textAlign: 'center' }}>Error loading events: {error.message}</div>
        </div>
      // </DashboardLayout>
    );
  }
      ...
      <div className='events-dashboard'>
        ...
        <div className='events-grid'>
          {filteredEvents.map((event) => (
            <EventCard key={event.id} event={event} expandedEvent={expandedEvent} setExpandedEvent={setExpandedEvent} />
          ))}
        </div>

        {filteredEvents.length === 0 && (
          <div style={{ padding: '20px', textAlign: 'center', color: '#6b7280' }}>{filter ? 'No events match your filter' : 'No events available'}</div>
        )}
          ...
          <div className='event-modal-overlay' onClick={() => setExpandedEvent(null)}>
            <div className='event-modal-content' onClick={(e) => e.stopPropagation()}>
              <div className='event-modal-header'>
                ...
                <button className='event-modal-close' onClick={() => setExpandedEvent(null)}>
                  ×
                </button>
              </div>
              ...
              <div className='event-modal-body'>
                <div className='event-modal-info'>
                  ...
                  <div className='event-details'>
                    <span className={`status-badge status-${expandedEventData.status}`}>{expandedEventData.status.toUpperCase()}</span>
                    <span className='event-meta'>Created: {new Date(expandedEventData.created_at).toLocaleString()}</span>
                    <span className='event-meta'>{expandedEventData.posts_count} posts</span>
                  </div>
                </div>
